...
static Nfa_t Re_thompsonDoit (Nfa_t nfa,Re_t e)
{
  ...
  case RE_KIND_ALT:{
    ...
    int leftStart = nfa->start;
    int leftAccept = nfa->accept;

    Re_thompsonDoit(nfa, p->right);
    int rightStart = nfa->start;
    int rightAccept = nfa->accept;

    int start = nextNum();
    int accept = nextNum();
   
    Nfa_addEdge(nfa, start, leftStart, EPS);
    Nfa_addEdge(nfa, start, rightStart, EPS);
    
    Nfa_addEdge(nfa, leftAccept, accept, EPS);
    Nfa_addEdge(nfa, rightAccept, accept, EPS);
